...
### W. Brent Thorne<sup>1</sup> & Dewey Dunnington<sup>2</sup>
...
# Tutorial 3 .black[Creating Visualizations using ggplot]
...
Bring in the data provided by Dewey Dunnington.
...
The Grammar of Graphics (the "gg" in "ggplot") is a way of describing a graphic that is derived from data, which in R is done using the `ggplot()` function and its many friends.

--

Unlike other plotting functions, `ggplot()` builds graphics from the data up (rather than starting with a template of a graphic and working backward).

---
## Using ggplot

Before we can use `ggplot` functionality we need to use the skills learned in Tutorial 2 where we filtered our data.

--

See if you can use `filter()` on the `halifax_geochem` data to create the `pockwock_data` and `pockwock_major_data` variable.
